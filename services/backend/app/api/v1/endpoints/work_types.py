...
async def create_work_type(payload: WorkType, current_user: UserInDB = Depends(get_current_user_with_scopes([1, 2]))):
...
async def read_work_type(id: int = Path(..., gt=0), user: UserInDB = Depends(get_current_user)):
