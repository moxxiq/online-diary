...
async def create_class(payload: Class, current_user: UserInDB = Depends(get_current_user_with_scopes([1, 2]))):
...
async def read_class(id: int = Path(..., gt=0), user: UserInDB = Depends(get_current_user)):
