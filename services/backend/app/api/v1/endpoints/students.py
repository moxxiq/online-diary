...
async def create_student(payload: Student, current_user: UserInDB = Depends(get_current_user_with_scopes([1]))):
...
async def read_student(user_id: int = Path(..., gt=0), user: UserInDB = Depends(get_current_user)):
