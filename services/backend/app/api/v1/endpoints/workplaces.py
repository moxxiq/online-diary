...
async def create_workplace(payload: Workplace, current_user: UserInDB = Depends(get_current_user_with_scopes([1, 2]))):
...
async def read_workplace(id: int = Path(..., gt=0), user: UserInDB = Depends(get_current_user)):
    """
...
async def get_all_student_workplaces_with_details(student_id: int = Path(..., gt=0), current_user: UserInDB = Depends(get_current_user_with_scopes([1, 2, 3]))):
