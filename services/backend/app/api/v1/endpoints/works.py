...
async def create_work(payload: Work, current_user: User = Depends(get_current_user_with_scopes([1, 2]))):
...
async def update_work(payload: WorkContent, id: int = Path(..., gt=0), current_user: User = Depends(get_current_user_with_scopes([1, 2]))):
...
async def read_work(id: int = Path(..., gt=0), current_user: User = Depends(get_current_user_with_scopes([1, 2, 3]))):
...
async def delete_work(id: int = Path(..., gt=0), current_user: User = Depends(get_current_user_with_scopes([1, 2]))):
    work_in_db = await crud.works.get(id)
    if not work_in_db:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Work not found")
